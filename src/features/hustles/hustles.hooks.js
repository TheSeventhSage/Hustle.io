...
export function useMyHustles(params = {}) {
  const activeTab = useHustlesStore((s) => s.activeTab)
  return useQuery({
    queryKey: queryKeys.hustles.mine({ ...params, status: activeTab }),
    queryFn:  () => hustlesService.getMyHustles({ ...params, status: activeTab }),
    staleTime: 60 * 1000,
  })
}
...
export function useCreateHustle() {
  ...
  return useMutation({
    ...
    onSuccess(data) {
      ...
      toastSuccess('Hustle created successfully!')
      navigate(`/hustles/${data.data.id}`)
    },
    onError(err) {
...
export function useMarkComplete() {
  ...
  return useMutation({
    ...
    onSuccess(_, id) {
      queryClient.invalidateQueries({ queryKey: queryKeys.hustles.detail(id) })
      queryClient.invalidateQueries({ queryKey: queryKeys.hustles.mine() })
      toastSuccess('Hustle marked as complete.')
    },
    onError(err) {
...
export function useSaveHustle() {
  ...
  return useMutation({
    ...
    onSuccess(_, { saved }) {
      ...
      toastSuccess(saved ? 'Hustle removed from saved.' : 'Hustle saved!')
    },
  })
}
