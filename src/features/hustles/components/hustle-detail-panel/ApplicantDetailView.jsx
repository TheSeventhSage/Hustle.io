...
export function ApplicantDetailView({ applicant, onBack, onClose }) {
    // flow states: 'idle' | 'debit_confirm' | 'payment' | 'payment_success' | 'reject_modal' | 'rejected'
    ...
    return (
        <>
            ...
            {/* Body — payment screen or applicant details */}
            {flow === 'payment' ? (
                <PaymentScreen
                    applicant={applicant}
                    onBack={() => setFlow('idle')}
                    onSuccess={() => setFlow('payment_success')}
                />
            ) : (
                <div className="flex-1 overflow-y-auto overscroll-contain px-5 sm:px-7 py-6">
                    ...
                    <p className="text-[12px] text-text-4 leading-relaxed mb-7">
                        By accepting this offer you will be requested to make full payment for the price the hustler indicated.
                    </p>
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="flex-1 h-12 bg-primary hover:bg-primary-sat text-white text-[14px] font-bold rounded-full transition-colors"
                        >
                            Accept Offer
                        </button>
                        <button
                            onClick={() => setFlow('reject_modal')}
                            className="flex-1 h-12 border-2 border-[var(--color-brand-grey)] text-[var(--color-brand-grey)] text-[14px] font-bold rounded-full hover:bg-[var(--color-mist)] transition-colors"
                        >
